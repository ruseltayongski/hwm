<script setup lang="ts">
    ...
    if(router_path === '/admin') {
        nav_name.value = 'Dashboard'
    } else if(router_path === '/laboratory') {
        nav_name.value = 'Laboratory'
    } else if(router_path === '/user_laboratory') {
        nav_name.value = 'Laboratory'
    }

    const team_2 = ref(Team2);
    const logo_spotify = ref(LogoSpotify);
    const emit = defineEmits(['searchEmployee']);

    const handleSearchEmployee = () => {
        emit('searchEmployee', search_keyword.value)
    }
</script>
<template>
    <!-- Navbar -->
    <nav class="relative flex flex-wrap items-center justify-between px-0 py-2 mx-6 transition-all shadow-none duration-250 ease-soft-in rounded-2xl lg:flex-nowrap lg:justify-start" navbar-main navbar-scroll="true">
        ...
        <nav>
            ...
            <li class="text-sm pl-2 capitalize leading-normal text-slate-700 before:float-left before:pr-2 before:text-gray-600 before:content-['/']" aria-current="page">{{ nav_name }}</li>
            </ol>
            <h6 class="mb-0 font-bold capitalize">{{ nav_name }}</h6>
        </nav>
